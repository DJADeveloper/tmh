...
const WebMaintenanceService = ({ service }) => {
  const webMaintenanceData = data.webDevelopmentServices[5];
  const { title, info, features, benefits, status, items } = webMaintenanceData;
  const settings = {
    infinite: false,
    speed: 500,
    slidesToShow: 1,
...
export default WebMaintenanceService;
