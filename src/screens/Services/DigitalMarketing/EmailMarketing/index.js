...
const EmailMarketingService = ({ service }) => {
  const emailMarketingData = data.digitalMarketingServices[5];
  const { title, info, features, benefits, status, items } = emailMarketingData;
  const settings = {
    infinite: false,
    speed: 500,
    slidesToShow: 1,
